...
onAuthStateChanged(auth, async (user) => {
    ...
    try {
        ...
        if (snap.exists()) {
            ...
            // Populate Top Nav
            if(document.getElementById("uWard")) document.getElementById("uWard").innerText = `Ward ${data.wardNumber || "--"}`;
...
document.getElementById("resetPasswordBtn")?.addEventListener("click", async () => {
    ...
    if (confirm(`Send a password reset email to ${currentUserEmail}?`)) {
        ...
        try {
            ...
            btn.disabled = false;
            btn.innerHTML = '<i class="bi bi-key me-2"></i>Send Password Reset Email';
        }
    }
});
